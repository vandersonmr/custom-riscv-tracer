...
This is the rv8, a RISC-V emulator,  modified to collect RISC-V compressed traces. Originally, -l dumps every single emulated instruction in a verbose mode into stdout. In this version, -l dumps every single instruction into a compressed file in a more organized way. These compressed files can be read and write using the trace_io.h/cpp files in src/traceio. 
